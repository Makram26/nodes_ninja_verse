...
export default function Home() {
  ...
  const checkNFT = async (wallet: any) => {
    ...
    let num = 0;
    for (let c in nfts) {
      if (Hashlist.includes(nfts[c].mint)) {
        console.log("mint",nfts[c].mint)
        // console.log("redirect to next page");
        let address=nfts[c].mint
        num = 1;
        navigate("/all-record", {
          state: { id: 1, name: wallet,mintAddress:address  },
        });
      }
    }
    if (num == 0) {
      alert("Please Renew Your NFT First!");
    }
