...
export default function AllRecord() {
  ...
  const [states, setStates] = useState("Frankfurt");
  ...
  const GenerateNOde = async () => {
    // let wallet = "kSP1HFm4viVFwwPbifF3SwD4q7wtX6gSxzuiCdhvhvv"
    let wallet = Location.state.name;
    let mintAddress = Location.state.mintAddress;
    ...
    try {
      ...
      const data = await fetch(url, {
        ...
        body: JSON.stringify({ ip, states, wallet,mintAddress }),
      });
                            ...
                            curElem.map((job) =>
                              job.active == true ? (
                                <tr key={job._id}>
                                  ...
                                  <td>Frankfurt
                                    {/* {job.location == "154.53.49.46"?"New York":"Frankfurt"} */}
                                    </td>
